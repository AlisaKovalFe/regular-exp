...
describe("hasSNILS", () => {
    ...
    // мой тест
    it("returns true if it finds a SNILS in non standart formats", () => {
        expect(hasSNILS("The number is 23460014222" || "The number is 234.600.142.22" || "The number is 013--605--876 94")).toBe(true);
    });
...
describe("grabAllSNILS", () => {
    ...
    // мой тест
    it("returns a collection of just one SNILS", () => {
        let allSNILS = grabAllSNILS("The number is 350-802-074 94");
...
describe("hideAllSNILS", () => {
    ...
    // здесь делать доп тест для проверки передачи строки с единственным СНИЛС бесполезно, так как функция преобразовывает строку в строку
});
